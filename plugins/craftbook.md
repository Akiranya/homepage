...
电梯（![电梯](../assets/images/plugins/cb-elevator.png ':size=16') 点击看大图）只是一些处在不同高度的 *木牌*，所以使用木牌就可以创建一个电梯啦。当然你还需要为电梯建造框架😊。关于电梯木牌的 *坐标*，除`y`*（高度）*之外，`x`与`z`*（平面）*都必须一致，这样木牌才能在楼层之间联系起来。
