...
如果企划指南有需要强制措施才能实现的规则（比如玩家只能在规定时间之后开始建造），请直接联系小米说明企划指南的需求。小米会考虑什么样的技术指南才可以保证企划指南按期望进行，然后分享在这里。
...
- 主持人/风纪委可以使用`/lp search`指令来查看一个权限组下有哪些玩家，一般用来查看现有的工匠、建筑师和设计师玩家有哪些（详见[简单管理指令](/staff/cmds-simple.md#lp)）
